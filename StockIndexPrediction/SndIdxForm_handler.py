...
class SndIdxForm_handler(QDialog, SndIdxFormClass):
    ...
    def ClickInitQttn(self):
        ...
        try:
            ...
            print("초기화 시작:", CodeDef.INDEX_STK_CD, StrDt, StrMn, CodeDef.INIT_END_DT_DEFAULT, CodeDef.INIT_END_MN_DEFAULT)
            # 기존데이터 삭제
            self.gDBH.deleteMnQttn("KOSPI",CodeDef.INDEX_STK_CD, StrDt, StrMn, CodeDef.INIT_END_DT_DEFAULT, CodeDef.INIT_END_MN_DEFAULT)

            # 데이터 수신 (빈분봉이 없기 때문에 따로 분봉채우기를 할 필요가 없다.)
            self.gAPIH.InitMnQttn(CodeDef.MKT_TP_CD_INTERNAL, CodeDef.INDEX_STK_CD, StrDt, StrMn, CodeDef.INIT_END_DT_DEFAULT, CodeDef.INIT_END_MN_DEFAULT)
            print("초기화 종료")
